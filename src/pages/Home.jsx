  ...
  useEffect(() => {
    const fetchLists = async () => {
      try {
        ...
        if (tm.status === "fulfilled") setTrendingMovies(tm.value.data.results);
        if (trm.status === "fulfilled") setTopMovies(trm.value.data.results);
        if (ts.status === "fulfilled") setTrendingSeries(ts.value.data.results);
        if (trs.status === "fulfilled") setTopSeries(trs.value.data.results);
      } catch (err) {
        console.log(err);
      }
    };
